...
#    def nobs(self): return self.nx+self.withSinCos
    ...
    def reset(self, fixed=False, x0=None):
        ''' Reset the state of the environment to x0 '''
        if fixed: np.random.seed(42)
        ...
        #if self.nx==4:
            #x0=x0.reshape(4,1)#soluzione raffazzonata che sembra funzionare
        self.x = x0.copy()
        self.r = 0.0
    ...
    def dynamics(self, x, u, display=True):
        ...
        for i in range(self.NDT):
            ...
            M   = self.data.M
            b   = self.data.nle
            #a   = (inv(M)*(u-self.Kf*v-b))#.reshape(4,1)
    ...
    def dynamicsEmu(self, u, display=True):
        '''
        Dynamic function: x,u -> xnext=f(x,y).
        Put the result in x (the initial value is destroyed). 
        Also compute the cost of taking this step.
        ...
        xfake[:self.nq] = self.mp(q)
        xfake[self.nq:] = np.clip(v,-self.vmax,self.vmax)
